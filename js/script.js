...
function formatTexto(tipo, texto) { // Se formatea el texto a minúsculas sin espacios o mayúsculas con espacios
...
function mostrarCarrito() {
    ...
    if (precioTotal != 0) { // Se muestra el carrito
        ...
        carrito.forEach((e, i) => {
            ...
            li.cassName = `servicio-${i}`
            li.innerHTML = `<strong>${e.servicio}</strong> Plan ${e.plan} = $${formatNum(e.precio)}`
            serv.appendChild(li);
...
cargarServicios().then(() => {
    cotizacionDolar().then(() => {
        ...
        let categorias = servicios.map(servicio => servicio.categoria); // Se guardan las categorías de productos
        categorias = [...new Set(categorias)] // Se eliminan las cat duplicadas
        ...
        servicios.forEach((s, i) => { // Se muestran los productos
            ...
            if (i % 2 == 0) { // Se ordenan de un lado o de otro
                $card.innerHTML = `
